...
drivers <- list(
  ...
  , iI = 0         ##<< input of mineral N
  , iIP = 0        ##<< input of mineral labile P
)
...
x0 <- c( #aE = 0.001*365
  ...
  , R = 1100                ##<< C rich substrate
  , RN = 1100/parms0$cnIR   ##<< N rich substrate 
  , RP = 1100/parms0$cpIR   ##<< P rich substrate 
  , L = 100                 ##<< C poor substrate
  , LN = 100/parms0$cnIL    ##<< N poor substrate 
  , LP = 100/parms0$cpIL    ##<< P poor substrate 
  , I =  0.01                ##<< inorganic pool gN/m2
  , IP =  0.01               ##<< inorganic pool gN/m2, no P limitation
...
resAgg <- resC %>% select(time, decNLR, decNE, decNB) %>% gather(key = "variable", "value", -time)
ggplot( resAgg, aes(time, value, color = variable, linetype = variable)) + geom_line()
...
Note, also that due apparent CUE is lower than the intrinsic catabolic CUE, $\epsilon = 0.5$,
...
(including respiration by predation)
yields similar estimates as those based on modeled microbial uptake.
