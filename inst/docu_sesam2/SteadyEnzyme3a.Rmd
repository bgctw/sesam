...
some equations have been simplified and computaiton of some intermediate
variables have been skipped.

That element limits microbial growth for which least C equivalent is available
for biomass synthesis and associated catabolic respriation after accumlating the
...
&= \beta_B /\epsilon \left[ \nuN (\operatorname{dec}_L/\beta_L + \operatorname{dec}_R/\beta_R + \kappa_E a_E B/\beta_E) + u_{\operatorname{imm,Pot}}- a_E B/\beta_E  \right] \\
&= \beta_B /\epsilon \left[ \nuN (\operatorname{dec}_L/\beta_L + \operatorname{dec}_R/\beta_R) + u_{\operatorname{imm,Pot}} - (1 - \nuN \kappa_E) a_E B/\beta_E  \right] \\
\\
