...
Here, we explore instead of tracking enzymes, we track microbial community either
attacking R or L litter, by modelling $\alpha$ as a state variable. The computation
of $\alpha_{Target}$ is based on current revenue, and hence current $\alpha$.

Microbial community can only change as fast as microbial turnover.
$$
\frac{d\alpha}{dt} = \tau (\alpha_{Target} - \alpha) 
$$

## Model versions
- SEAM2: enzyme explicit (soil enzyme allocation model)
- SESAM3s: enzyme in steady state but explicitely tracking $alpha$
...
parmsModel <- list(
  ...
  ,iB = 0.38 * 10.57 #0.0110068*365   ##<< potential immobilization flux rate (immoPot = iB I)
  ,l = 0.96       #0.00262647*365       ##<< leaching rate of mineralN l I
  ,nu = 0.9     # microbial N use efficiency accounting for apparent minceralization of N during uptake in heterogeneous soils
...
drivers <- list(
        ...
        ,iP = 10.57 #0.0289652*365          ##<< plant uptake of inorganic N (I), currently as absolute flux
...
scenarios <- tibble::tibble(
  scenario = c("Explicit","Sesam3s","NeglectEnzFlux")
  ,mod = c(derivSeam2, derivSesam3s, derivSesam3s)
  ,x0  = list(x0Explicit, x0SteadyA, x0SteadyA)
  ,parms = list(parms0, parms0, parms0Noenz)
)
...
```{r}
runFaceSimulation <- function(scen, t1S =20, t2I = 50, t3S = 50, parms = parms0){
...
Model Sesam3s matches the dynamics of the explicit SEAM model at this time scale.
