...
parmsModel <- list(
        cnB = 11
        ,cnE = 3.1     # Sterner02: Protein (Fig. 2.2.), high N investment (low P) need to have low C/N for CO2 increase scenario
        ...
        ,kNB = 0.8     ##<< amount of recycling enzyme turnover by biomass (added to assimilable, i.e. uptake instead of R)
...
Next the simulation rus are performed.
```{r}
runFaceSimulation <- function(scen, t1S =10, t2I =50, t3S = 50, parms0=parms0){
  # spinup to steady state
  times <- seq(0,500, length.out=101)
  #setwd("../..");loadPkg()
  . <- scen
  res <- res1 <- as.data.frame(lsoda( .$x0, times, .$mod, parms=parms0))
  xE <- unlist(tail(res,1))
  #
  # t1S years of steady state recorded
  res <- res1S <- as.data.frame(lsoda( xE[1:length(.$x0)+1], 1:t1S, .$mod, parms=parms0))
  #
  # t2I years of increased input
  parmsC2 <- within(parms0, { # more C in litter, but not more N 
    iL <- iL*fInputInc
    cnIL <- cnIL*fInputInc
  }) 
  ...
  res <- res3S <- as.data.frame(lsoda( xE2[1:length(.$x0)+1], times, .$mod, parms=parms0))
