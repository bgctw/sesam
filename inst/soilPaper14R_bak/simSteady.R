quit()      # do not execute on make, see predecessor simSteady2
...
simfCNGraph <- function(
    ...
    resLC <- lapply( cnLs, function(cnL){ 
                ...
                #resL <- lapply( "Revenue", function(scen){
                            ...
                            res <- res1 <- as.data.frame(lsoda( x0N, times, derivSeam1, parms=parmsInit))
                            xE <- unlist(tail(res,1))
                            #plotResSeam1(res, "topright", cls = c("B10","respO","Mm","Rr","Lr","alpha100"))
                            #trace(derivSeam1, recover) #untrace(derivSeam1)
                            #tmp <- derivSeam1(0, xE[1:length(x0)+1], parmsInit)
                            xE
                        })
    ...
    p8 <- ggplot( dsp, aes(x=cnL, y=value, col=scen) ) + geom_line(size=1) + 
    ...
    p8a <- ggplot( dsp, aes(x=cnL, y=value, col=scen) ) + geom_line(size=1) + 
            #facet_grid(Measure~., scales = "free", labeller= label_parsed ) + 
            facet_wrap(~Measure, scales = "free_y" ) +
            #facet_wrap(~Measure ) +
    ...
    p8b <- ggplot( dsp, aes(x=cnL, y=value, col=Pool) ) + geom_line(size=1) + 
            #facet_grid(Measure~., scales = "free", labeller= label_parsed ) + 
...
simCO2Increase <- function(
    ...
    resAll <- lapply( c("Revenue","Fixed"), function(scen){
            ...
            # 10 yr steady state
            res <- res1S <- as.data.frame(lsoda( xE[1:length(x0)+1], 1:t1S, derivSeam1, parms=parmsInit))
            ...
            res <- res2I <- as.data.frame(lsoda( xE[1:length(x0)+1], times, derivSeam1, parms=parmsC2))
            res2I$time <- res2I$time +t1S 
            xE2 <- unlist(tail(res2I,1))
            plotResSeam1(res, "topright", cls = c("B10","respO","Mm","Rr","Lr","alpha100"))
            #trace(derivSeam1, recover)        #untrace(derivSeam1)
            tmp <- derivSeam1(0, xE2[1:length(x0)+1], parmsC2)
            
            # 30 yr normal input
            times <- seq(0,t3S, length.out=101)
            res <- res3S <- as.data.frame(lsoda( xE2[1:length(x0)+1], times, derivSeam1, parms=parmsInit))
...
### Simulated decrease of input to 1/100 during years 10-210
    ...
    resAll <- lapply( c("Revenue","Fixed"), function(scen){
                ...
                res <-  as.data.frame(lsoda( x0, times, derivSeam1, parms=parmsInit))
                #res <- res1f <- as.data.frame(lsoda( x0, times, derivSeam1, parms=within(parms0, useFixedAlloc<-TRUE) ))
                ...
                res <- as.data.frame(lsoda( unlist(xESteadyP[1:length(x0)+1]), times, derivSeam1, parms=parmsC2))
                ...
                res <- res3Sc <- as.data.frame(lsoda( xE2[1:length(x0)+1], times, derivSeam1, parms=parmsC2))
                ...
                res <- res3S <- res3Sp <- as.data.frame(lsoda( x0P[1:length(x0)+1], times, derivSeam1, parms=parmsC2))
...
### Simulated decrease of input to 1/100 during years 10-210
    ...
    resAll <- lapply( c("Revenue","Fixed"), function(scen){
                ...
                resc$scen <- scen
                plotResSeam1(resc, "topright", cls = c("B10","respO","Mm","Rr","Lr","alpha100"))
                resc$tvrR <- 1/(parmsInit$kR * resc$limE1)
                resc
            })
    resScen <- do.call( rbind, resAll)
