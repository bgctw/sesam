...
test_that("same as seam for fixed substrates", {
  ...
  res <- derivSesam3s(0, x0, parmsInit)
  times <- seq(0, 2100, length.out = 2)
  #times <- seq(0,2100, length.out = 101)
  resTest <- as.data.frame(lsoda( x0, times, derivSesam3s, parms = parmsFixedS))
  ...
  .tmp.f <- function(){
    ...
    derivSesam3s(0, xEExp[c(2,5:9)], within(parmsInit, isRecover <- TRUE))
  }
  #
  # N limitation
  #times <- seq(0,2100, length.out = 101)
  resTest <- as.data.frame(lsoda( x0Nlim, times, derivSesam3s, parms = parmsFixedS))
  resTestNoEnz <- as.data.frame(lsoda(
...
test_that("same as seam with substrate feedbacks", {
  ...
  resTest <- as.data.frame(lsoda( x0, times, derivSesam3s, parms = parmsInit))
  resTestNoEnz <- as.data.frame(lsoda(
    x0, times, derivSesam3s
  ...
  .tmp.f <- function(){
    derivSesam3s(0, xETest[2:8], within(parmsInit, isRecover <- TRUE))
    derivSeam3a(0, xEExp[2:10], within(parmsInit, isRecover <- TRUE))
    derivSesam3s(0, xETest, within(parmsInit, isRecover <- TRUE))
    derivSesam3s(0, xpESteady, within(parmsInit, isRecover <- TRUE))
  }
  #
  # N limitation
  resTest <- as.data.frame(lsoda( x0Nlim, times, derivSesam3s, parms = parmsInit))
  resTestNoEnz <- as.data.frame(lsoda(
    x0Nlim, times, derivSesam3s
    , parms = within(parmsInit, isEnzymeMassFlux <- FALSE)))
  ...
  resTest <- as.data.frame(lsoda( x0CNLim, times, derivSesam3s, parms = parmsInit))
  resTestNoEnz <- as.data.frame(lsoda(
