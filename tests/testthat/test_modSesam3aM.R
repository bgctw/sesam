...
parms0 <- list(
  ...
  ,plantNUpAbs = Inf   ##<< no absolute limit of plant uptake
...
test_that("same as seam for fixed substrates", {
  ...
  res <- derivSesam3aM(0, x0, parmsInit)
  resExp <- derivSesam3a(0, x0, parmsInit)
  expect_equal(res[[1]], setNames(resExp[[1]], names(x0)), tolerance = 1e-12)
  #res[[1]] - resExp[[1]]
  times <- seq(0, 2100, length.out = 2)
  #times <- seq(0,2100, length.out = 101)
  resTest <- computeOutputsSesam3(lsoda(
    x0, times, derivSesam3aM, parms = parmsFixedS), parmsFixedS)
  resExp <- as.data.frame(lsoda( x0, times, derivSesam3a, parms = parmsFixedS))
  xETest <- unlist(tail(resTest,1))
  xEExp <- unlist(tail(resExp,1))
  expect_equal( xETest["alphaC"], xEExp["alphaC"], tolerance = 1e-6)
  expect_equal( xETest[2:7], xEExp[c(2:7)], tolerance = 1e-6)
  .tmp.f <- function(){
  ...
  # N limitation
  #times <- seq(0,2100, length.out = 101)
  resTest <- as.data.frame(computeOutputsSesam3(
    lsoda( x0Nlim, times, derivSesam3aM, parms = parmsFixedS), parmsFixedS))
  resTestNoEnz <- as.data.frame(computeOutputsSesam3(lsoda(
    x0Nlim, times, derivSesam3aM
    , parms = within(parmsFixedS, isEnzymeMassFlux <- FALSE))
    , within(parmsFixedS, isEnzymeMassFlux <- FALSE)
    ))
  #resExpOld <- as.data.frame(lsoda(
  #  x0NlimSeam3, times, derivSeam3a, parms = parmsFixedS))
  resExp <- as.data.frame(lsoda(x0Nlim
    , times, derivSesam3a, parms = parmsFixedS))
  xETest <- unlist(tail(resTest,1))
  xETestNoEnz <- unlist(tail(resTestNoEnz,1))
  xEExp <- unlist(tail(resExp,1))
  expect_equal( xETest["alphaN"], xEExp["alphaN"], tolerance = 1e-6)
  expect_equal( xETest[2:7], xEExp[c(2:7)], tolerance = 1e-6)
  expect_equal( xETestNoEnz[2:7], xEExp[c(2:7)], tolerance = 0.1)
})
...
test_that("same as seam with substrate feedbacks", {
  ...
  resTest <- computeOutputsSesam3(lsoda(
    x0, times, derivSesam3aM, parms = parmsInit), parmsInit)
  resExp <- as.data.frame(lsoda( x0, times, derivSesam3a, parms = parmsInit))
  xETest <- unlist(tail(resTest,1))
  xEExp <- unlist(tail(resExp,1))
  expect_equal( xETest["alphaC"], xEExp["alphaC"], tolerance = 1e-6)
  expect_equal( xETest[2:7], xEExp[c(2:7)], tolerance = 1e-6)
  ...
  # N limitation
  resTest <- as.data.frame(computeOutputsSesam3(
    lsoda( x0Nlim, times, derivSesam3aM, parms = parmsInit), parmsInit))
  resTestNoEnz <- as.data.frame(computeOutputsSesam3(lsoda(
    x0Nlim, times, derivSesam3aM
    , parms = within(parmsInit, isEnzymeMassFlux <- FALSE))
    , within(parmsInit, isEnzymeMassFlux <- FALSE)
  ))
  resExp <- as.data.frame(lsoda(x0Nlim, times, derivSesam3a, parms = parmsInit))
  xETest <- unlist(tail(resTest,1))
  xETestNoEnz <- unlist(tail(resTestNoEnz,1))
  xEExp <- unlist(tail(resExp,1))
  expect_equal( xETest["alphaN"], xEExp["alphaN"], tolerance = 1e-6)
  ...
  resTest <- as.data.frame(computeOutputsSesam3(
    lsoda( x0CNLim, times, derivSesam3aM, parms = parmsInit), parmsInit))
  resTestNoEnz <- as.data.frame(computeOutputsSesam3(lsoda(
    x0CNLim, times, derivSesam3aM
    , parms = within(parmsInit, isEnzymeMassFlux <- FALSE))
    , within(parmsInit, isEnzymeMassFlux <- FALSE)
  ))
  resExp <- as.data.frame(lsoda(x0CNLim, times, derivSesam3a, parms = parmsInit))
  xETest <- unlist(tail(resTest,1))
  xETestNoEnz <- unlist(tail(resTestNoEnz,1))
  xEExp <- unlist(tail(resExp,1))
  ...
  expect_equal( xETest[2:7], xEExp[c(2:7)], tolerance = 1e-3)
  expect_equal( xETestNoEnz[2:7], xEExp[c(2:7)], tolerance = 0.1)
  rbind(xEExp[c(2:7)], xETest[2:7], xETestNoEnz[2:7])
