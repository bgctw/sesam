...
test_that("same as seam for fixed substrates", {
  ...
  test_that("same as seam for fixed substrates", {
    ...
    expect_true( all(abs(xESteady[2:7] - xEExp[c(2,5:9)])/pmax(1e-10,xEExp[c(2,5:9)]) < 0.1))
  ...
  expect_true( all(abs(xESteady[2:7] - xEExp[c(2,5:9)])/pmax(1e-10,xEExp[c(2,5:9)]) < 0.05))
  .tmp.f <- function(){
    derivSeam2(0, xEExp[2:9], within(parmsInit, isRecover <- TRUE))
    derivSesam2(0, xEExp[c(2,5:9)], within(parmsInit, isRecover <- TRUE))
  }
