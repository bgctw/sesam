...
Janssens10, Zaehle11}. The links between nutrient cycles are especially strong
...
studies at the pore scale, which is relevant to microbes, show the important
effect of community adaptation and their emerging effects on element cycling
...
system could be soil of a laboratory incubation or a soil layer outside the
rhizosphere.
...
litter pool. Inorganic N pool was kept steady at $I=0.4 \mathrm{gN}$.
...
used fixed resource carbon of $L=100 \mathrm{gC}$, $R=400 \mathrm{gC}$.
...
decade. It simulated the amandmend of an very available substrate, specifically
...
greater potential for plants to avoid progressive N limitation
...
implemented with the SEAM model. First, a fraction of microbial uptake, termed
uptake mineralization $\Phi_u$, is apparently mineralized that accounts for
...
during microbial turnover, $\Phi_{\operatorname{tvr}}$. The N
dynamics depends also on fluxes across the system boundary,
namely input of litter $iL$ to the soil organic matter ($SOM$), input of inorganic N $iI$, leaching, and
plant uptake of inorganic N.
...
The long-term patterns of simulated SOM turnover are robust because the longer
term activity is determined proportions in enzyme production. Hence, the
disagreement in timescale of the priming effect does not impair the modelling of
longer-term microbial community controls on SOM cycling.
...
I hypothize, that the holistic approach is more suitable for upscaling
\citep{Wutzler13}.
