...
\\
\frac{dI}{dt} &= +i_I -k_{IP} -l I +\Phi + \operatorname{tvr}_N
\text{,} 
\end{align}
\end{subequations}

\noindent
where the balance of the inorganic nitrogen pool $I$ sums inorganic inputs
$i_I$, plant uptake $k_{IP}$, leaching $l I$, the
mineralization-immobilization flux $\Phi$, and mineralization of a part of microbial turnover
$\operatorname{tvr}_N$.

Potential nitrogen uptake is the sum of uptake from DOM and the potential
immobilization flux ($u_{\operatorname{imm,Pot}}$). Uptake from DOM is assumed
equal to influxes to DOM.
\begin{equation}
\label{eq:uN}
\begin{align}
...
\\ 
u_N &= u_{N,OM} + u_{\operatorname{imm,Pot}}
\text{,}
\end{align}
\end{equation}
