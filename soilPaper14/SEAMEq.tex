...
\remarkSZ{why not use C and N consistently here?}{}
...
If the carbon balance for biomass synthesis, $\operatorname{syn}_B$ (eq.
...
met. Hence, the microbial carbon balance can become negative where microbial biomass starves
and declines.
...
Potential nitrogen uptake is the sum of organic N uptake and the potential
immobilisation flux ($u_{\operatorname{imm,Pot}}$). Uptake from DOM is assumed
equal to influxes to DOM times the apparent nitrogen use efficiency $\nu$.
\begin{subequations}
\label{eq:uN}
\begin{align}
...
The nitrogen available for biomass synthesis is the difference of microbial nitrogen
...
nitrogen is mineralised, $M_{\operatorname{Imb}}$, so that the mass
balance is closed.
\begin{subequations}
\label{eq:imbalance}
\begin{align}
...
the potential immobilisation flux and excess nitrogen mineralization.
If microbes are limited by carbon availability, $\Phi_B$ will be positive, whereas with 
substrate nitrogen limitation, $\Phi_B$ will be a negative flux, corresponding to
N immobilisation.
With uptake nitrogen limitation, i.e. required immobilisation is larger than
