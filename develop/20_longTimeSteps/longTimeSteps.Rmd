...
with increase C input by litter, i.e increased C from decomposition with
same N return, the system runs into N limitation.
...
```{r}
dxc <- deriv_inorg_nitrogen(0, x1, parmsCinc)
dxc
```
```{r}
res2c %>% 
...
```{r}
x1c <- unlist(res2c[nrow(res2c),1 +seq_along(x0)])
x1c
dxc <- deriv_inorg_nitrogen(0, x1c, parmsCinc)
dxc
```

### Time stepped solution without rate adjustment
```{r}
...
```{r echo=FALSE}
rbind(
  mutate(res2c, solver = "ode")
  ,mutate(resStep1, solver = "timestep")
) %>% 
  select(time, solver, IN, B) %>% 
  pivot_longer(!c(time, solver)) %>% 
  ggplot(aes(time, value, color = solver)) + geom_line() +
  facet_wrap(~name, scales = "free_y")

```

The numerical error is most present in IN pool with oscillating behavior
due to overshoots.

### Time stepped solution with rate adjustment
```{r}

resStep1 <- solveDiscreteAdj(x1, 0:6, deriv_inorg_nitrogen, parmsCinc, 
                             tstep = 2) 
```
