...
```{r}
x0 <- c( #aE = 0.001*365
  B = 85                    ##<< microbial biomass in C units
  , R = 1                   ##<< C residue OM
...
ggplot( resC, aes(timeDay, L)) + geom_line()
#ggplot( resC, aes(timeDay, R)) + geom_line()
# microbial turnover
...
ggplot( resC, aes(timeDay, B)) + geom_line()
```
...
# CN
resCM <- res1 %>% 
  mutate(betaL = LN/L, betaR = RN/R, OM = L + R + B
         , OMN = LN + RN + B/parms0$cnB, betaOM = OMN/OM
...
it consists almost solely of residue pool R, and assumed trickling inputs into the 
fresh pool.
...
The L is not depleted yet, but cannot deliver enough carbon to sustain biomass.
...
x02["B"] <- 1e-8
x02["L"] <- L
x02["LN"] <- L/parms0$cnIL
x02["LP"] <- L/parms0$cpIL
