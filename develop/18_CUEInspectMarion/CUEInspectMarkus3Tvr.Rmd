...
\begin{align}
f_L(L)g(B) &= I \\
dB/dt &= \epsilon I + \epsilon f_R(R)g(B) - \tau B^\delta \\
\end{align}
$$
...
$$
\begin{align}
0 &= \epsilon I +\epsilon \, k_R R^* g(B^*) - \tau {B^*}^\delta \\
0 &= \epsilon I + \epsilon \, k_R \frac{\epsilon I}{k_R (1-\epsilon) g(B^*)} g(B^*) - \tau {B^*}^\delta \\
0 &= \epsilon I + \epsilon \, \frac{\epsilon I}{(1-\epsilon) }  - \tau {B^*}^\delta \\
B^* &= \left( \left( 1 + \frac{\epsilon}{(1-\epsilon)} \right) \frac{\epsilon I}{\tau} \right)^{1/\delta}
     = \left( \frac{\epsilon I}{\tau (1-\epsilon)} \right)^{1/\delta} \\
\end{align}
$$
...
R^* &=  \frac{\epsilon I}{k_R (1-\epsilon) g(B^*)}  
    ...
    =  \frac{\epsilon I}{k_R (1-\epsilon)}  \left( k_M {B^*}^{-1} + 1\right) \\
    &= \frac{\epsilon I}{k_R (1-\epsilon)}  \left( k_M \left( \frac{\epsilon I}{\tau (1-\epsilon)} \right)^{-1/\delta} + 1\right) \\
    &= \frac{1}{k_R}  \left( k_M \tau^{1/\delta} \left( \frac{\epsilon I}{1-\epsilon} \right)^{1-1/\delta} + \frac{\epsilon I}{1-\epsilon}\right) \\
\end{align}
$$
...
L^* &= f_L^{-1}\left( I / g(B^*) \right)    
    ...
    = \frac{I}{k_L}  ( k_M {B^*}^{-1} +1 ) \\
    &= \frac{I}{k_L}  \left( k_M \left( \frac{\epsilon I}{\tau (1-\epsilon)} \right)^{-1/\delta} +1 \right) \\
    &= \frac{I}{k_L}  \left( k_M \left( \frac{\tau (1-\epsilon)}{\epsilon I} \right)^{1/\delta} +1 \right) \\
\end{align}
$$
...
#delta <- 1.5; kL <- kR*5
delta <- 1.2; kL <- 5
B <- (eps*I/(tau*(1 - eps)))^(1/delta)
kM <- as.numeric(quantile(B, 0.8))
tkm <- tau*kM
R <- R1 <- eps*I/(kR*(1 - eps))*(kM + B)/B
R <- R2 <- 1/kR*(kM*tau^(1/delta)*(eps*I/(1 - eps))^(1 - 1/delta) + eps*I/(1 - eps))
L <- L1 <- I/kL*(kM + B)/B
L <- L2 <- I/kL*(kM*(tau*(1 - eps)/(eps*I))^(1/delta) + 1)
...
```{r echo=FALSE}
p <- list(I = I, tau = tau, kR = kR, kL = kL, kM = kM, delta = delta, eps = 0.4)
unlist(p)
```
