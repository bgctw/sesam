...
scenarios0 <- tibble::tibble(
  scenario = c("Explicit","Sesam3s","Sesam3B")
  ,mod = c(derivSeam3a, derivSesam3s, derivSesam3B)
...
parmsFixedR <- within(parms0, {
  iL <- 0.1
  isFixedR <- TRUE
  #fixedI anyway kIPlant <- iL / cnIL*2 # balancing N inputs by plant uptake
  })
...
res3L <- as.data.frame(lsoda( x03L, timesMon, derivSesam3s, parms = parmsFixedR))
res3H <- as.data.frame(lsoda( x03H, timesMon, derivSesam3s, parms = parmsFixedR))
res <- res3 <- rbind(
