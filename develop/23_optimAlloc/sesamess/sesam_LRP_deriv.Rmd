...
u_decomp <- function(alpha, dLorR, o_enz, p){
...
ds2 <- bind_rows(
  mutate(ds, Biomass="High", u_L = u_decomp(alpha, dL, o_enz,  within(p, B <-2))),
  mutate(ds, Biomass="Low", u_L = u_decomp(alpha, dL, o_enz,  within(p, B <-1)))
)
...
u_biomin <- function(alpha, dP, o_enz, p){
  dP*p$k_mn/o_enz/((p$k_mn + p$e_P)^2 + alpha * p$a_E * p$B * ((p$k_mn + p$e_P)))
}
ds2 <- bind_rows(
  mutate(ds, PlantProduction="none", u=u_biomin(alpha, dL, o_enz,  p)),
  mutate(ds, PlantProduction="same magnitude", u=u_biomin(alpha, dL, o_enz, within(p, e_P <- k_mn/2))),
)
...
```{r include=FALSE}
compute_utilities <- function(alpha3,dL,dR,dP,p,o_enz=0.2){
  c(
      u_decomp(alpha3["L"], dL, o_enz, p), 
      u_decomp(alpha3["R"], dR, o_enz, p), 
      u_biomin(alpha3["P"], dP, o_enz, p)
    )
}
compute_total_utilities <- function(alpha3,dL,dR,dP,p,o_enz=0.2){
  u = compute_utilities(alpha3,dL,dR,dP,p,o_enz)
  sum(alpha3 * u)*p$a_E*p$B
}
calc_alpha3_relative = function(dL,dR,dP,p,o_enz=0.2) {
  ...
  for (i in 1:5) {
    alpha3 = u1/sum(u1)
    u1 = compute_utilities(alpha3,dL,dR,dP,p,o_enz)
