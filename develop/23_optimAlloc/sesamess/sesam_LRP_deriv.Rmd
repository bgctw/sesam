...
p2 <- pivot_longer(dsAlpha, !c(dL,Approach,Biomass), names_to="Enzyme") %>% 
  mutate(Enzyme = fct_relevel(Enzyme, "R","P")) %>% 
  #ggplot(aes(dP, value, color=Enzyme, linetype=Approach)) + geom_line() +
  ggplot(aes(dL, value, color=Enzyme, linetype=Approach, size=Biomass)) + geom_line() +
  xlab(bquote('Potential return from depolymerizing labile ('*g~ m^-2~s^-1*')')) +
