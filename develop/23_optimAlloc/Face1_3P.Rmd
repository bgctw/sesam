...
scenarios <- tibble::tibble(
  ...
  ,parms = list(
    ...
    within(parms0, {isRelativeAlpha<-TRUE; kmNR <- 3.261561})
    )
)
scenarios$x0 <- list(x0)
scenarios$mod <- list(derivSesam3P)
tmp <- structure(mycolors[1:nrow(scenarios)], names = scenarios$scenario)
colScaleApproach <- scale_colour_manual(name = "Model variant",values = tmp )
linetypeScaleApproach <- scale_linetype_manual(name = "Model variant",values =  structure(c("solid","longdash","dotted","dashed")[1:nrow(scenarios)], names = scenarios$scenario))
scenarios
```
...
replace_labels <- function(labels, labelmap) {
  sapply(labels, function(label){
    if (is.null(varlabelmap[[label]])) label else varlabelmap[[label]]
  })
}
#replace_labels(c("a",names(labelmap)), labelmap)
ggplot_vars <- function(resScen, vars, labelmap = list(alphaR="alpha[R]", "R_norm"="R[norm]")) {
  dsp <- 
    #resc %>% 
    resScen %>% 
...
p4b <- p4 +
  theme(legend.position = c(0.93,1.15), legend.justification = c(1,1))
ggsave("sesamalloc_FACE2.pdf", p4b, width = figWidthCol, height = figHeightCol) 
```
...
.tmp.f <- function(){
  ...
  head(tmp)$R + head(tmp)$L # targetSOM = 3314.14
}
...
fcost_kmNR <- function(kmNRnew, scen){
  ...
  res <- runSteady( scen$mod[[1]], scen$x0[[1]], p ) 
  (tail(res$R,1)+tail(res$L,1) - 3314.14)^2
}
.tmp.f <- function(){  
  #"Optimal","Relative","Derivative"
...
.tmp.f <- function(){  
  ...
  scen2 <- filter(scenarios, scenario == "Relative_kmNR")  # kR = 1/32.6147
