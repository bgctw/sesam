...
scenarios <- tibble::tibble(
  ...
  ,parms = list(
    ...
    within(parms0, {isRelativeAlpha<-TRUE; kR <- kRP <- 1/33.80632}),
    within(parms0, {isRelativeAlpha<-TRUE; kmNR <- 3.478608})
    )
)
...
vars <- c("L","R","alphaR","B","IN","plantNUp","PhiNB","PhiNTotal","limN") 
...
.tmp.f <- function(){  
  ...
  c(1/parms$kR, 1/kR$minimum)  # 31.4 yrs vs 33.80632
}
.tmp.f <- function(){  
  #"Optimal","Relative","Derivative"
  scen <- filter(scenarios, scenario == "Relative")  
  ...
  kmNR <- resOpt$minimum
  c(parms$kmN, kmNR)  # 3.0 vs 3.478608
}
```
