...
scenarios <- tibble::tibble(
  scenario = c("Optimal","Relative","Derivative")
  ,parms = list(
...
```{r}
runFaceSimulation <- function(scen, t1S =20, t2I = 40, t3S = 70) {
  . <- scen
...
.tmp.f <- function(){
  ...
  scen <- unlist(filter(scenarios, scenario == "Optimal"), recursive = FALSE)
  scen <- unlist(filter(scenarios, scenario == "Relative"), recursive = FALSE)
  scen <- unlist(filter(scenarios, scenario == "Derivative"), recursive = FALSE)
  resScen <- runFaceSimulation( scen ) %>% mutate(scenario = scen$scenario)
}
resScen <- scenarios %>% nest(data=!.data$scenario) %>% mutate(res=map(.data$data, function(df){
  scen <- unlist(df, recursive = FALSE)
  runFaceSimulation(scen)
