...
colScale <- scale_colour_manual(name = "Approach",values = mycolors)
linetypeScale <- scale_linetype_manual(name = "Approach",values = c("solid","longdash","dashed","dotted"))
...
```{r}
vars <- c("L","R","alphaR") 
p4 <- resScen %>% filter(scenario != "Relative") %>% 
  ggplot_vars(vars)+ colScale + linetypeScale +
    theme(axis.title.y = element_blank()) +
    theme(legend.title = element_blank())
p4
