...
linetypeScale <- scale_linetype_manual(name = "Variant",values = c("solid","longdash","dotted"))
...
resScen <- scenarios %>% nest(data=!.data$scenario) %>% mutate(res=map(.data$data, function(df_scen){
...
ggplot_vars <- function(resScen, vars) {
  dsp <- 
    ...
    mutate(scenario = fct_relevel(.data$scenario, c("Optimal","Derivative","Relative"))) %>% 
