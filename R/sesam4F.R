...
derivSesam4F <- function(
  ...
  xOrig <- xvec
  xvec <- pmax(unlist(xvec),1e-16)      # no negative masses
  ...
  dLN <- -decL/cnL*x$rel[[L]] + parms$iL/parms$cnIL*parms$relIL
  dLP <- -decL/cpL*x$rel[[L]] + parms$iL/parms$cpIL*parms$relIL
