...
derivSesam3b <- function(
  ...
  dI <-  +parms$iI  - plantNUp  - leachN  + PhiU  + PhiNB  + PhiTvr
  #
  ...
  resDeriv <- structure(as.numeric(
    c( dB, dR, dRN, dL, dLN, dI, dAlpha["R"]))
    ,names = c("dB","dR","dRN","dL","dLN","dI","dAlphaR"))
...
computeSesam3bAllocationPartitioning <- function(
  ...
  cost <- (kmkN + alpha*synEnz) *
    (wELim["C"] + wELim["N"]/beta["E"])
  returnL <- dS["L"]*(wELim["C"] + wELim["N"]/beta["L"])
  returnR <- dS["R"]*(wELim["C"] + wELim["N"]/beta["R"])
