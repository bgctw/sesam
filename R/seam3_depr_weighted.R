...
# gC/m2 and gN/m2, /yr
#
#seam3a with computation of alpha_target computed differently
#  on a revenue: (limitation-weighted return)/(limitation weighted investment)
#  does not make sense after all and does not resolve the problem of
#  depending near the arbitraty weighing near co-limitation

derivSeam5a <- function(
  ...
  getAlphaTarget <- function(wCLim, wNLim){
    weDenom = (wCLim + wNLim/cnE)
    weL = (wCLim + wNLim/cnL)/weDenom
    weR = (wCLim + wNLim/cnR)/weDenom
    revL <- decLp / (parms$kNL) / (parms$kmL + EL) * weL
    revR <- decRp / (parms$kNR) / (parms$kmR + ER) * weR
    sumRev <- revL + revR
    alphaTarget <- revL/sumRev
  }
  alphaTarget <- getAlphaTarget(wCLim, wNLim)
  alphaC <- getAlphaTarget(1,0)
  alphaN <- getAlphaTarget(0,1)
  # Revenue strategy
  #alphaTargetR <- revR/sumRev
  alpha <- x["alpha"]
