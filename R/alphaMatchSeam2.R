calcMatchAlphaSeam2 <- function(
        ...
        , decPotR   ##<< numeric scalar: current potential decomposition of residue pool (with out accounting for enzyme limitations)
        , decPotL   ##<< numeric scalar: current potential decomposition of litter pool (with out accounting for enzyme limitations)
        , rMaint    ##<< numeric scalar: maintenance respiration
        , parms     ##<< list of parameter values including substrate affinities kmR, kmL, and anabolic efficiency eps
...
attr(calcMatchAlphaSeam2, "ex") <- function(){
    if( FALSE ){
        ...
        c(cnOpt, synC/synN) # check that C/N ratio of fluxes for biomass synthesis equal the target
    }
}
