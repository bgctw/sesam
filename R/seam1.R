...
derivSeam1 <- function(t,x,parms){
    ...
    .tmp.f <- function(){
        #decR <- parms$kR  * ER / (parms$km + ER) * 1000#*  x["R"]
        #decL <- parms$kL  * EL / (parms$km + EL) * 1000#*  x["L"]
        uscER <- parms$kN*x["ER"]        # production necessary to balance decay of ER (uptake steady carbon ER)
        uscEL <- parms$kN*x["EL"]     
        # average cnE Required for enzyme production according to current concentrations (usc weighted)
        # cnE <- (parms$cnER*uscER + parms$cnEL*uscEL ) / (uscER+uscEL)
        # C balance under C limitation
        effER <- decR/uscER
        effEL <- decL/uscEL
        alphaC2 <- effER/(effER + effEL)
        
        
        
        synBC <- synB       # used in calculation of alpha
        uNReq <- synE/cnE + synB/parms$cnB
        effERN <- (decR/cnR)/(uscER/parms$cnER)
        effELN <- (decL/cnL)/(uscEL/parms$cnEL)
        alphaN <- effERN/(effERN + effELN)
        uNSynB <- uN - synE/cnE  
        synBN <- uNSynB*parms$cnB  # C for both growth and growth respiration
        uCReq <- (synE + synBN)/parms$eps + rM  
        if( uN < uNReq ){
            # N limitation
            synB <- synBN
            # average cnE reqruired according to enzyme allocation accoording to N efficiencies
            #cnE <- alpha*parms$cnER + (1-alpha)*parms$cnEL
    ...
    if( isTRUE(parms$isAlphaMatch) ){
        ...
        # calcMatchAlphaEnz
        alpha <- calcMatchAlpha( E=ER+EL, decPotR=decRp, decPotL=decLp, respMaint=rM 
                ,cnOpt=cnOpt
                , cnR = cnR, cnL=cnL
                , parms=parms)
    }
