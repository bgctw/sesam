...
.tmp.f <- function(){
    ...
    derivEezy6(0, x0, parmsInit)
    times <- seq(0,2100, length.out=101)
    #times <- seq(0,10000, length.out=101)
    res <- res1 <- as.data.frame(lsoda( x0, times, derivEezy6, parms=parmsInit))
    ...
    tmp <- derivEezy6(0, xE[1:length(x0)+1], parmsInit)
    ...
    res <- res2S <- as.data.frame(lsoda( xE[1:length(x0)+1], times, derivEezy6, parms=parmsC2))
    xE2 <- unlist(tail(res,1))
    plotRes(res, "topright", cls = c("B10","respO","Mm","S1r","S2r","alpha100"))
    #trace(derivEezy5, recover)        #untrace(derivEezy5)
    derivEezy6(0, xE2[1:length(x0)+1], parmsC2)
    
    res <- res3S <- as.data.frame(lsoda( xE2[1:length(x0)+1], times, derivEezy6, parms=parmsInit))
    plotRes(res, "topright", cls = c("B10","respO","Mm","S1r","S2r","alpha100"))
    xE3 <- tail(res,1)
    ...
    res <- res2S <- as.data.frame(lsoda( xE[1:length(x0)+1], times, derivEezy6, parms=parmsC0))
